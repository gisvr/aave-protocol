...
let ethUSD = "500";
let usdETH = "0.2" //1美元对应的 ETH

let _purchaseAmount = (new BN("200")).mul(ethDecimalBN); //374 临界值

let ethAddr = "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE";

describe("AAVE Liquidation ETH", function () {
    ...
    before(async () => {
        ...
        let mockToken = [TokenBAT, TokenUSDC]
    ...
    it("alice,bob,sender depoist 1000 DAI, BAT, USDC", async () => {
        ...
        let _tokenBal =await web3.eth.getBalance(alice)
        console.log(new BN(_tokenBal).div(ethDecimalBN).toString())
    ...
    it("sender borrow ETH ", async () => {
        ...
        let _tokenBal =await web3.eth.getBalance(sender)
        ...
        console.log("ETH",availableBorrowsETH.toString())
        // 将可借的 EHT转换成对应的资产
        ...
        let tokenAmount =await web3.eth.getBalance(sender)
        // expect(borrowAmount).to.be.bignumber.equal(new BN(tokenAmount).sub(new BN(_tokenBal)));
        // 检查 利率模型
    ...
    it("sender calculate Available Collateral To Liquidate ETH", async () => {
    ...
    it("aave liquidation repay ETH, harvest USDC", async () => {
        let _collateral= this.USDC.address
        let _reserve = ethAddr;
        let userAccountData = await this.lpContractProxy.getUserAccountData(sender) 
        ...
        let liquidDai =new BN(await web3.eth.getBalance(liquid))
        ...
        let _liquidDai =new BN(await web3.eth.getBalance(liquid))
