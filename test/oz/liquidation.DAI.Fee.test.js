...
describe("AAVE Liquidation DAI", function () {
    ...
    it("alice,bob,sender depoist 1000 DAI, BAT, USDC", async () => {
        ...
        await this.USDC.approve(this.lpCoreAddr, allowAmount,{from:alice})

      
        await this.lpContractProxy.deposit(this.DAI.address, allowAmount, 0)
        await this.lpContractProxy.deposit(this.BAT.address, allowAmount, 0,{from:bob})
        await this.lpContractProxy.deposit(this.USDC.address, new BN(1000).mul(usdDecimalBN), 0,{from:alice} )
    ...
    it("sender borrow USDC ", async () => {
        this.timeout(50000)
        let _reserve = this.USDC.address;
        let _tokenBal =await this.USDC.balanceOf(sender);
        ...
        // 将可借的 EHT转换成对应的资产
        let borrowAmount = availableBorrowsETH.mul(usdDecimalBN).div(_priceEth);

        console.log("borrow USDC",borrowAmount.div(usdDecimalBN).toString(),borrowAmount.toString(),)
        ...
        // 检查 用户得到的Token
        let tokenAmount =await this.USDC.balanceOf(sender);
        ...
        ///-----------------------------------------
        borrowFee = borrowFee.add(usdDecimalBN.mul(new BN(1000)))// 补充位操作 9-6

        console.log(borrowFee.toString())
        let feeAmount = borrowFee.mul(usdDecimalBN).div(_priceEth);
        ///-----------------------------------------
    ...
    it("aave asset devaluation DAI", async () => {
        this.timeout(50000)
        ...
        expect(userCollateralBalanceETH).to.be.bignumber.eq(userAccountData.totalCollateralETH,"用户总计：抵押资产"); 
    ...
    it("sender calculate Available Collateral To Liquidate DAI", async () => {
        ...
        expect(borrowBalances[0].mul(new BN("25")).div(new BN("10000"))).to.be.bignumber.equal(originationFee.sub(new BN(1)));
        // 验证产生的借出费用 
    ...
    it("aave liquidation repay USDC, harvest DAI", async () => {
        let _collateral= this.DAI.address
        let _reserve = this.USDC.address;
        let userAccountData = await this.lpContractProxy.getUserAccountData(sender) 
        ...
        let liquidDai = await this.USDC.balanceOf(liquid)
        console.log("USDC Before", liquidDai.toString())

        let borrowBal=await this.aDAI.balanceOf(sender);
        let liquBal=await this.aDAI.balanceOf(liquid);
        // console.log("Borrow aUSDC Before",borrowBal.toString())
        // console.log("liquid aUSDC Before",liquBal.toString()) 

        
        await this.USDC.approve(this.lpCoreAddr, _purchaseAmount.mul(new BN(10)),{from:liquid})
        ...
        let _liquidDai = await this.USDC.balanceOf(liquid)
       

 
        console.log("liquid USDC Before - After %s", liquidDai.sub(_liquidDai).div(usdDecimalBN).toString()) //
 
        // 未计算费用
        let _borrowBal=await this.aDAI.balanceOf(sender);
        let _liquBal=await this.aDAI.balanceOf(liquid); // 清算用户本来有的资产
        ...
        console.log("fee aDAI %s,  %s",avaiableCollateral.collateralAmount.div(ethDecimalBN).toString(), avaiableCollateral.collateralAmount.toString());
        console.log("borrow aDAI ", _borrowBal.div(ethDecimalBN).toString(),_borrowBal.toString())
        console.log("liquid aDAI ",_liquBal.div(ethDecimalBN).toString(),_liquBal.toString());
