...
module.exports = class AaveMarket extends Market {
    constructor(web3) {
        super(web3)
    }
    ...
    calculateUserGlobalData(account, userGlobalData, priceUsd) {
        console.log("UserGlobalData----------------", account)
        for (let key in userGlobalData) {
            if (key.length < 3) continue

            let userAccountVal = userGlobalData[key].toString()
            let ethKeys = {
                "totalLiquidityBalanceETH": "总共流动的ETH余额",
                "totalCollateralBalanceETH": "总计抵押ETH余额",
                "totalBorrowBalanceETH": "总计借出ETH余额",
                "totalFeesETH": "总计Fee余额",

                "currentLtv": "资产的 LTV",
                "currentLiquidationThreshold": "资产的清算阈值",
                "healthFactor": "健康系数",
            ...
            //  "healthFactorBelowThreshold": "健康系数"
